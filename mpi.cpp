...
Node * buildTreeSerial (std::vector<Point> &points, int start, int end, int dim, int axis) {
    ...
    node -> setLeft(buildTreeSerial(points, start, median - 1, dim, _axis));
    node -> setRight(buildTreeSerial(points, median + 1, end, dim, _axis));
